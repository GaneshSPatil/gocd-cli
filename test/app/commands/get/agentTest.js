...
const agentGetAPIPath = path.resolve('app/api/agents/get.js');
...
const stubbedAgentsGetAPI = sinon.stub();
const stubbedLogger = {
  'info': sinon.stub(),
  'error': sinon.stub()
};
...
stubbedRequires[agentGetAPIPath] = stubbedAgentsGetAPI;
stubbedRequires[loggerPath] = stubbedLogger;
stubbedRequires[argumentValidatorPath] = stubbedArgumentValidator;

const getCommand = proxyquire(path.resolve('app/commands/get/agent.js'), stubbedRequires);
...
describe('Agents Get Command', () => {
  ...
  it('should set the command on the program', () => {
    getCommand(program);
    assert.that(command.getCall(0).args[0]).is.equalTo('get_agent');
  });

  it('should set the description on the program', () => {
    getCommand(program);
    assert.that(description.getCall(0).args[0]).is.equalTo('get an agent');
  });

  it('should make api call to agent get on action', () => {
    const options = {'uuid': 'agent-uuid'};

    stubbedAgentsGetAPI.resolves({});
    getCommand(program);
    return action.getCall(0).args[0](options).then(() => {
      assert.that(stubbedAgentsGetAPI.callCount).is.equalTo(1);
    });
  });
  ...
  it('should log result of action on success', () => {
    ...
    stubbedAgentsGetAPI.resolves(agent);
    getCommand(program);
    return action.getCall(0).args[0](options).then(() => {
  ...
  it('should log result of action on error', () => {
    ...
    stubbedAgentsGetAPI.rejects(error);
    getCommand(program);
    return action.getCall(0).args[0](options).then(() => {
  ...
  it('should validate name argument option', () => {
    ...
    stubbedAgentsGetAPI.resolves(pipeline);
    getCommand(program);
    return action.getCall(0).args[0](options).then(() => {
      assert.that(stubbedArgumentValidator.validateNotNull.getCall(0).args[0]).is.equalTo('agent-uuid');
      assert.that(stubbedArgumentValidator.validateNotNull.getCall(0).args[1]).is.equalTo('-u, --uuid <agent_uuid>');
    });
  });
