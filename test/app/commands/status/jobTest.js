...
const statusCommand = proxyquire(path.resolve('app/commands/status/job.js'), stubbedRequires);
...
describe('Job Status Command', () => {
  ...
  it('should set the command on the program', () => {
    ...
    assert.that(command.getCall(0).args[0]).is.equalTo('status_job');
  });

  it('should set the description on the program', () => {
    statusCommand(program);
    assert.that(description.getCall(0).args[0]).is.equalTo('status of a job');
  });
  ...
  it('should return the status of the specified job', () => {
    const cctrayResponse = [
      {
        ...
        'webUrl': 'http://127.0.0.1:8153/go/pipelines/up42/1/up42_stage/1/job/1',
        'pipelineName': 'up42',
  ...
  it('should throw error when status for the specified stage could not be found', () => {
    ...
    statusCommand(program);
    return action.getCall(0).args[0](options).then(() => {
      assert.that(stubbedLogger.error.getCall(0).args[0]).is.equalTo('\n error: Job \'up42 :: up42_stage_2 :: up42_job\' not found\n');
    });
  });

  it('should throw error when status for the specified job could not be found', () => {
    const cctrayResponse = [
      {
        'name': 'up42 :: up42_stage :: up42_job',
        'activity': 'Sleeping',
    ...
    statusCommand(program);
    return action.getCall(0).args[0](options).then(() => {
      assert.that(stubbedLogger.error.getCall(0).args[0]).is.equalTo('\n error: Job \'up42 :: up42_stage :: up42_job_2\' not found\n');
    });
  });
  ...
  it('should validate name argument option', () => {
    ...
    statusCommand(program);
    return action.getCall(0).args[0](options).then(() => {
      assert.that(stubbedArgumentValidator.validateNotNull.getCall(2).args[0]).is.equalTo('up42');
      assert.that(stubbedArgumentValidator.validateNotNull.getCall(2).args[1]).is.equalTo('-p, --pipeline_name <pipeline_name>');

      assert.that(stubbedArgumentValidator.validateNotNull.getCall(1).args[0]).is.equalTo('up42_stage');
      assert.that(stubbedArgumentValidator.validateNotNull.getCall(1).args[1]).is.equalTo('-s, --stage_name <stage_name>');

      assert.that(stubbedArgumentValidator.validateNotNull.getCall(0).args[0]).is.equalTo('up42_job');
      assert.that(stubbedArgumentValidator.validateNotNull.getCall(0).args[1]).is.equalTo('-j, --job_name <job_name>');
    });
  });
